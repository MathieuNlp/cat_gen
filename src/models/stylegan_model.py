...
class MappingNetwork(nn.Module):
    def __init__(self, features, n_layers):
        ...
        for i in range(n_layers):
            layers.append(EqualiedLinear(features, features))
...
class Generator(nn.Module):
    ...
    def forward(self, w, input_noise: List[Tuple[Optional[torch.Tensor], Optional[torch.Tensor]]]):
...
class GeneratorBlock(nn.Module):
    ...
    def forward(self, x, w, noise: Tuple[Optional[torch.Tensor], Optional[torch.Tensor]]):
...
class StyleBlock(nn.Module):
    def __init__(self, d_latent, in_features, out_features):
        super().__init__()
        self.to_style = EqualiedLinear(d_latent, in_features, bias=1)
...
class ToRGB(nn.Module):
    def __init__(self, d_latent, features):
        super().__init__()
        self.to_style = EqualiedLinear(d_latent, features, bias=1)
...
class Conv2dWeightModulate(nn.Module):
    def __init__(self, in_features, out_features, kernel_size, demodulate: float=True, eps=1e-8):
        super().__init__()
    ...
    def forward(self, x, s):
        ...
        weights = weights*s
        # result of shape [batch_size, out_features, in_features, kernel_size, kernel_size]
        if self.demodulate:
...
class Discriminator(nn.Module):
    def __init__(self, log_resolution, n_features=64, max_features=512):
        ...
        self.classif_layer = EqualiedLinear(2*2*final_features, 1)
...
class EqualiedLinear(nn.Module):
...
class PathLengthPenalty(nn.Module):
    ...
    def forward(self, w, x):
        ...
        gradients, *_ = torch.autograd.grad(outputs=output,
                                            inputs=w,
                                            grad_outputs=torch.ones(output.shape, deivce=device),
                                            create_graph=True)
        ...
        self.exp_sum_a.mul_(self.beta).add(mean, alpha=1-self.beta)
        self.steps.add_(1.)
        return loss
