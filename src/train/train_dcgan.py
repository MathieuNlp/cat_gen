...
import src.models.dcgan_model as dcgan
...
def train(dataloader, generator, discriminator, real_label, fake_label, fixed_noise, device, num_epochs):
    ...
    for epoch in range(num_epochs) : 
        for batch, data in enumerate(dataloader):
            ...
            real_batch_device = data[0].to(device) # take the image tensor of size 3,128,128
            ...
            iters+=1
        if (epoch % 30 == 0):
            torch.save(generator.state_dict(), save_PATH+f"new_generator_checkpoint_{epoch}.pt")
            torch.save(discriminator.state_dict(), save_PATH+f"new_discriminator_checkpoint_{epoch}.pt")
...
def visu_progress(img_list):
    ...
    ani.save('generation.mp4', writer=writervideo)
